    ...
    <nav className="bg-[#111827] border-b border-gray-800 sticky top-0 z-50">
      ...
      {isOpen && (
        <div className="lg:hidden bg-[#111827] border-t border-gray-800 px-6 py-6 flex flex-col gap-5">
          <a href="#learning-path" onClick={() => setIsOpen(false)} className="text-gray-300 hover:text-white text-lg font-medium">Jalur Pembelajaran</a>
          <a href="#materi" onClick={() => setIsOpen(false)} className="text-gray-300 hover:text-white text-lg font-medium">Program</a>
          <div className="relative w-full mt-2">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4" />
            <Input 
              placeholder="Cari materi..." 
              onChange={(e) => onSearchChange(e.target.value)} // Tetap berfungsi di mobile
              className="bg-[#1F2937] border-gray-700 pl-10 text-white w-full"
            />
          </div>
        </div>
      )}
